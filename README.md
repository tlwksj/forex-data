...
1. Scripts          
   ...
   **Data Sources:**      
   ...
   Both sources provide the data in JSON format, which facilitated the cleaning process. The script holds two functions one that facilitates the convertion from EU rates to usd/mxn rates and the second to aid the writing onto a CSV file. For the historical part, the script verifies that the CSV file for the date has not been created, and then proceeds to extract the last 5 years of data for the given date. In case that the date has been extracted (which would occur once this repository is at least a year old), the script only extracts the current date's data and appends it to the CSV, with the purpose of adding onto the historical file. For the historical data, every date in the calendar has its own file, making sure that the data is easy to find and append to.

    b. Data cleaning script (clean_data.R)     
    This is a simple script that takes the newly updated script, cleans it (in case of a badly formatted response that was not catched by the extracting script), and writes it again to be used by the visalization script. 
